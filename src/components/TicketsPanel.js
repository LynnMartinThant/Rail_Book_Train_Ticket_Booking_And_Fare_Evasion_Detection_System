...
import ShowTicketByQR from './ShowTicketByQR';
...
/** Tickets + travel history (used on Profile). */
function TicketsPanel({ bookings, bookingsLoading, onBuyTicket }) {
        ...
        {history.length === 0 ? (
          ...
          <ul className="space-y-2">
            {history.map((b) => (
              ...
              >
                <div>
                  <p className="font-medium text-slate-900">{(b.journeyFromStation && b.journeyToStation) ? `${b.journeyFromStation} → ${b.journeyToStation}` : (b.trip ? `${b.trip.fromStation} → ${b.trip.toStation}` : '')}</p>
